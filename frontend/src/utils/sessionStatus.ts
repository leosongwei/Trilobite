...
// with a fixed priority: running (green, pulsing) > a cron still to fire
// (blue) > idle (gray). The dot class/title mapping lives next to the
// status logic so templates stay a one-line lookup.
...
export function sessionStatus(s: Session): SessionStatus {
  ...
  if (s.has_schedule) return 'pending'
  return 'idle'
}
...
export function statusDot(status: SessionStatus): { cls: string; title: string } {
  switch (status) {
    ...
    case 'pending':
      return { cls: 'pending-dot', title: 'cron scheduled' }
    case 'idle':
