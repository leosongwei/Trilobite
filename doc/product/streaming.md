...
* **流式回合统一重试**：任意 HTTP 错误、流无完成信号断开（无 `[DONE]`/finish reason）、或空完成（无 content 且无 tool_calls）都走同一重试循环（`_stream_turn`）——丢弃本回合部分输出、重发相同请求，每次重试广播 `status` 横幅（含异常原因）+ `turn_restart` 事件，上限 `max_stream_retries`（默认 10，含首次）。详见 [llm_transport.md](./llm_transport.md)。
