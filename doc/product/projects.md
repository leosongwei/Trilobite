...
* `POST /api/projects/{id}/rename` — 重命名项目 `{name}`（不存在返回 404）。
...
* **项目行操作**：行尾 hover 出现铅笔（重命名项目：名称变为行内输入框，Enter 确认、Esc 取消）、`+`（以项目的名称和工作目录创建 session 并归属于该项目，创建后自动选中）和 `×`（删除项目，确认后成员 session 保留为未分组）。
