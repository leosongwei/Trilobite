...
典型用例：等 CI/构建跑完再检查（`+10m`）、每天早上九点继续昨天的任务（`09:00`）、提醒用户到点做某事、轮询式任务（醒来检查、再睡）。
...
`until` 接受的格式（解析失败/时间非法时错误信息附带**当前本地时间**，模型可自行修正后重试）：

| 格式 | 语义 | 示例 |
|---|---|---|
| `YYYY-MM-DD HH:MM[:SS]` | 绝对时间 | `2025-06-01 09:00` |
| `YYYY-MM-DDTHH:MM[:SS]` | 同上（ISO 分隔符） | `2025-06-01T09:00` |
| `MM-DD HH:MM[:SS]` | 今年该时刻，已过则明年 | `06-01 09:00` |
| `HH:MM[:SS]` | 今天该时刻，已过则明天 | `09:00` |
| `+[n][s\|m\|h\|d]` | 相对时长 | `+30m`、`+2h`、`+1d`、`+90s` |
