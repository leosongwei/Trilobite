...
* `agent.py` -- 核心 Agent 类。管理会话生命周期：构建 system 消息（env 块 + `SYSTEM_PROMPT` + 工作目录 `AGENTS.md` + skills 清单），与 LLM 流式对话，循环执行 tool calls，支持用户 steering、上下文压缩、subagent 派生（`task` 工具）和 VLM 图片输入。详见 `doc/product/context_building.md`、`doc/product/streaming.md`、`doc/product/subagent.md`、`doc/product/skills.md`。
...
* `tool_call.py` - 工具注册与分发。维护全局工具列表，两个**虚拟工具**定义（仅 LLM 可见，执行逻辑在 Agent 中）：`TASK_TOOL_DEF`（`task`，派生并行 subagent）和 `SLEEP_UNTIL_DEF`（`sleep_until`，挂起会话到指定时间）；`execute_tool` 执行具体工具。
* `tools/tool.py` -- `Tool` 抽象基类（ABC），声明 `name`/`description`/`parameters` + 抽象 `execute` + `to_openai_tool`，所有具体工具继承它。
* `permission.py` - Agent 权限策略抽象。`AgentPermission` 基类承担两个职责：声明工具列表（`filter_definitions`）+ 拦截调用（`intercept`）。`tool_names` 声明**允许集**，通用 `intercept` 以它为唯一依据拦截调用；`advertised_tool_names` 声明**暴露集**，缺省值为允许集（暴露即允许）。主 agent 用 `PrimaryPermission`（全量工具 + `task`/`sleep_until` 虚拟工具），subagent 用**角色**（`ExploreSubagentPermission` 只读 / `GeneralSubagentPermission` 可编辑，派生时固化）。`task` 工具仅主 agent 暴露（硬性限一层嵌套）。详见 `doc/product/subagent.md`。
...
* `main.ts` -- 入口
* `App.vue` -- 根组件，布局 sidebar + chat + token bar；subagent 顶栏（返回父 session、类型标签、sealed 只读）；permission/subagent_permission 审批横幅
* `store.ts` - 全局状态：sessions、chat items、SSE 订阅流处理（`init` 重建对话、事件驱动 isStreaming、断线自动重连）、subagent 状态、VLM 开关；session 列表 3s 轮询
...
用户主动操作工作区文件的轻量 IDE 界面（仅主 session，sidebar 入口），与 agent 完全解耦：查看文件（highlight.js 高亮）、对比文件与指定 git 分支（默认 master，复用 `DiffView` 渲染 `DiffRow[]`）、textarea 编辑后直接落盘。文件树按目录懒加载（展开时请求 `fs/list`），git 仓库中被 ignore 的目录不出现，条目带 M/A/U/D 状态徽章。编辑保存不受 agent 运行状态限制，不产生对话历史、不走 permission 审批（用户操作，与终端改文件同语义）；路径统一走 `resolve_file_path` 边界检查，二进制/超 512KB/敏感文件拒绝。详见 `doc/product/file_manager.md`。
