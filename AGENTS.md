...
* `cli.py` -- 命令行交互模式（`trilobite -t` 新建 / `-c` 续接当前目录最新 session / `-s` 服务器为默认）。纯订阅者 + 渲染器：实例化 `Agent`、`attach_subscriber()` 订阅 broker 队列（与 SSE `/stream` 同构），把事件渲染到终端。类 bash REPL：常驻 stdin reader 协程喂 `asyncio.Queue`，IDLE 读输入、RUNNING 用 `asyncio.wait` 多路复用 broker 事件 / stdin（steering）/ Ctrl+C（cancel）。`-c` 按 `history.json` 的 mtime（最后存盘时间）找当前目录最新主 session 续接（加载历史、不回显）。不改动 agent/broker/工具层。详见 `doc/product/cli.md`。
...
* 更新完功能后，修改pyproject.toml中包版本的第三个数字。比如当前是1.0.13，那就改为1.0.14。
* 完成后提交PR
